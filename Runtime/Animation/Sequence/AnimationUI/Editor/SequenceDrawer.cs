...
namespace OSK
{
    ...
    public class SequenceDrawer : PropertyDrawer
    {
        ...
        public override float GetPropertyHeight(SerializedProperty property, GUIContent label)
        {
            ...
            Sequence.ObjectType targetType =
                (Sequence.ObjectType)property.FindPropertyRelative("TargetType").enumValueIndex;
            if (sequenceType == Sequence.Type.Animation)
            {
                Sequence.ObjectType objectType =
                    (Sequence.ObjectType)property.FindPropertyRelative("TargetType").enumValueIndex;
                float totalHeight = 0;

                if (objectType == Sequence.ObjectType.UnityEventDynamic)
                {
                    totalHeight = _height * 5 +
                                  EditorGUI.GetPropertyHeight(property.FindPropertyRelative("EventDynamic")) +
                ...
                if (property.FindPropertyRelative("TargetComp").GetSerializedValue<Component>() == null)
                    return _height * 6;

                if (objectType == Sequence.ObjectType.RectTransform)
                {
                    Sequence.RtTask rtTask =
                        (Sequence.RtTask)property.FindPropertyRelative("TargetRtTask").enumValueFlag;
                ...
                {
                    Sequence.TransTask transTask =
                        (Sequence.TransTask)property.FindPropertyRelative("TargetTransTask").enumValueFlag;
                ...
                {
                    Sequence.ImgTask imgTask =
                        (Sequence.ImgTask)property.FindPropertyRelative("TargetImgTask").enumValueFlag;
                ...
                {
                    Sequence.CgTask cgTask =
                        (Sequence.CgTask)property.FindPropertyRelative("TargetCgTask").enumValueFlag;
                    if (cgTask.HasFlag(Sequence.CgTask.Alpha)) totalHeight += _height * 3;
                }
                else if (objectType == Sequence.ObjectType.Camera)
                {
                    Sequence.CamTask camTask =
                        (Sequence.CamTask)property.FindPropertyRelative("TargetCamTask").enumValueFlag;
                ...
                {
                    Sequence.TextMeshProTask textMeshProTask =
                        (Sequence.TextMeshProTask)property.FindPropertyRelative("TargetTextMeshProTask").enumValueFlag;
                    if (textMeshProTask.HasFlag(Sequence.TextMeshProTask.Color)) totalHeight += _height * 3;
                    if (textMeshProTask.HasFlag(Sequence.TextMeshProTask.MaxVisibleCharacters))
                        totalHeight += _height * 3;
                }
            ...
            else if (sequenceType == Sequence.Type.UnityEvent)
            {
                ...
                return _height + (EditorGUI.GetPropertyHeight(property.FindPropertyRelative("Event")) +
                                  EditorGUIUtility.singleLineHeight);
            }
        ...
        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
        {
            ...
            Rect backgroundRect = new Rect(_backgroundRectExtra.x, position.y + _backgroundRectExtra.y,
                position.width + _backgroundRectExtra.width,
                GetPropertyHeight(property, label) + _backgroundRectExtra.height);
            ...
            if (sequenceType == Sequence.Type.Animation)
            {
                ...
                if ((Sequence.ObjectType)property.FindPropertyRelative("TargetType").enumValueIndex ==
            ...
            else if (sequenceType == Sequence.Type.UnityEvent)
            {
                ...
                property.FindPropertyRelative("IsUnfolded").boolValue //Fix for when the label is wrong
                    = EditorGUI.Foldout(nextPosition, property.FindPropertyRelative("IsUnfolded").boolValue,
                        "At " + time.ToString() + "s [UnityEvent]");
            }

            if (sequenceType != Sequence.Type.UnityEvent &&
                ((Sequence.ObjectType)property.FindPropertyRelative("TargetType").enumValueIndex !=
                 Sequence.ObjectType.UnityEventDynamic))
            {
            ...
            if (GUI.Button(
                    new Rect(position.x + position.width - _buttonWidth * 2, position.y - 3, _buttonWidth, _height),
                    "Start"))
            {
                property.FindPropertyRelative("TriggerStart").boolValue = true;
            }
            else if (GUI.Button(
                         new Rect(position.x + position.width - _buttonWidth, position.y - 3, _buttonWidth, _height),
                         "End"))
            {
            ...
            EditorGUI.PropertyField(nextPosition, property.FindPropertyRelative("SequenceType"),
                new GUIContent("Type"));
            //
            if (sequenceType == Sequence.Type.Animation)
            {
                ...
                EditorGUI.PropertyField(
                    new Rect(nextPosition.x, nextPosition.y, nextPosition.width * (0.61f), nextPosition.height),
                    property.FindPropertyRelative("EaseType"), new GUIContent("Ease")
                );

                EditorGUI.PropertyField(
                    new Rect(nextPosition.x + nextPosition.width * (0.61f), nextPosition.y,
                        nextPosition.width * (0.39f), nextPosition.height),
                    property.FindPropertyRelative("EasePower"), GUIContent.none
                );

                nextPosition.y += _height;
                EditorGUI.PropertyField(
                    new Rect(nextPosition.x, nextPosition.y, nextPosition.width, nextPosition.height),
                ...
                if (objectType == Sequence.ObjectType.RectTransform)
                {
                    ...
                    void DrawRtTask(string name)
                    {
                        ...
                        nextPosition.y += _height;
                        if (GUI.Button(new Rect(nextPosition.x, nextPosition.y, nextPosition.width / 4 - 5, _height),
                                "Set Start"))
                        {
                            if (name == "AnchoredPosition")
                                property.FindPropertyRelative(name + "Start").vector3Value =
                                    property.FindPropertyRelative("TargetComp").GetSerializedValue<RectTransform>()
                                        .anchoredPosition;
                        ...
                        EditorGUI.PropertyField(
                            new Rect(nextPosition.x + nextPosition.width / 4, nextPosition.y,
                                nextPosition.width * 3 / 4, _height),
                ...
                {
                    EditorGUI.PropertyField(nextPosition, property.FindPropertyRelative("TargetTransTask"),
                        new GUIContent("Task"));
                    ...
                    void DrawTransTask(string name)
                    {
                                ...
                                "Set Start"))
                        {
                            if (name == "LocalPosition")
                                property.FindPropertyRelative(name + "Start").vector3Value =
                                    property.FindPropertyRelative("TargetComp").GetSerializedValue<Transform>()
                                        .localPosition;
                            else if (name == "LocalScale")
                                property.FindPropertyRelative(name + "Start").vector3Value =
                                    property.FindPropertyRelative("TargetComp").GetSerializedValue<Transform>()
                ...
                {
                    EditorGUI.PropertyField(nextPosition, property.FindPropertyRelative("TargetImgTask"),
                        new GUIContent("Task"));
                ...
                {
                    EditorGUI.PropertyField(nextPosition, property.FindPropertyRelative("TargetCgTask"),
                        new GUIContent("Task"));
                ...
                {
                    EditorGUI.PropertyField(nextPosition, property.FindPropertyRelative("TargetCamTask"),
                        new GUIContent("Task"));
                ...
                {
                    EditorGUI.PropertyField(nextPosition, property.FindPropertyRelative("TargetTextMeshProTask"),
                        new GUIContent("Task"));
                    ...
                    void DrawTextMeshProTask(string name)
                    {
                                ...
                                "Set Start"))
                        {
                            ...
                            {
                                int maxVisibleCharactersStart = property.FindPropertyRelative("TargetComp")
                                    .GetSerializedValue<Transform>().GetComponent<TMP_Text>().maxVisibleCharacters;
                                int maxCharacters = property.FindPropertyRelative("TargetComp")
                                    .GetSerializedValue<Transform>().GetComponent<TMP_Text>().text.Length;
                                property.FindPropertyRelative(name + "Start").intValue =
                                    Mathf.Clamp(maxVisibleCharactersStart, 0, maxCharacters + 1);
                            }
                        }
                                ...
                                "Set End"))
                        {
                            ...
                            {
                                int maxVisibleCharactersEnd = property.FindPropertyRelative("TargetComp")
                                    .GetSerializedValue<Transform>().GetComponent<TMP_Text>().maxVisibleCharacters;
                                int maxCharacters = property.FindPropertyRelative("TargetComp")
                                    .GetSerializedValue<Transform>().GetComponent<TMP_Text>().text.Length;
                                property.FindPropertyRelative(name + "End").intValue =
                                    Mathf.Clamp(maxVisibleCharactersEnd, 0, maxCharacters + 1);
                            }
                        }
                    ...
                    Sequence.TextMeshProTask textMeshProTask =
                        (Sequence.TextMeshProTask)property.FindPropertyRelative("TargetTextMeshProTask").enumValueFlag;
                    if (textMeshProTask.HasFlag(Sequence.TextMeshProTask.Color))
            ...
            {
                nextPosition.y += _height;
                EditorGUI.PropertyField(nextPosition, property.FindPropertyRelative("Target"),
