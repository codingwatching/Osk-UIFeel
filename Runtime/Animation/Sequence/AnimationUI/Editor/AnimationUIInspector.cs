            ...
            foreach (Sequence sequence in animationUI.AnimationSequence)
            {
                ...
                if (animationUI.CurrentTime >= sequence.StartTime)
                {
                    float currentSideLoadingRectHeight = (sequence.Duration == 0)
                        ? currentRect.height
                        : Mathf.Lerp(0, currentRect.height,
                            (animationUI.CurrentTime - sequence.StartTime) / sequence.Duration);
                    Rect sideLoadingRect = new Rect(currentRect.x, currentRect.y, currentRect.width,
                        currentSideLoadingRectHeight);
                ...
                if (sequence.SequenceType == Sequence.Type.Animation)
                {
                    if (sequence.TargetComp != null)
                    {
                        ...
                        {
                            if (sequence.TargetComp.GetComponent<TMP_Text>() != null)
                                sequence.AtTime += " [TextMeshPro]";
                            else
                            {
                                sequence.TargetComp = null;
                            }
